...
def load_model(model_path, device):
    ...
    try:
        ...
        # Process model weights by removing the _orig_mod. prefix
        model_weights = {
...
def h(state, model):
    """
    Heuristic function: predict the distance from current state to target state.
    """
    with torch.no_grad():
...
def a_star_search(initial_state, model, cube, lam=0.6, max_iterations=200, N=1000):
    """
    Solve the Rubik's cube with A* search.
    """
    # Check whether the initial state is already the target state
    ...
    initial_state_tensor = torch.tensor(
        initial_state, device=next(model.parameters()).device
    ).long()
    initial_state_tensor = (
        torch.nn.functional.one_hot(initial_state_tensor, num_classes=6)
        .float()
        .view(-1)
    )
    ...
    f_score = {
        tuple(initial_state): lam * g_score[tuple(initial_state)]
        + h_score[tuple(initial_state)]
    }
    ...
    while open_set and iterations < max_iterations:
        iterations += 1
        # print(f"Current iteration: {iterations}, open set size: {len(open_set)}")
        ...
        # Collect all neighboring states
        neighbor_states = []
        ...
        for current_state_tuple in current_states:
            ...
            for action in cube.moves.keys():
                ...
                if (
                    next_state_tuple not in g_score
                    or tentative_g_score < g_score[next_state_tuple]
                ):
        ...
        if neighbor_states:
            ...
            neighbor_states_tensor = torch.tensor(
                neighbor_states, device=next(model.parameters()).device
            ).long()
            neighbor_states_tensor = (
                torch.nn.functional.one_hot(neighbor_states_tensor, num_classes=6)
                .float()
                .view(-1, 324)
            )
